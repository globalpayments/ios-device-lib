...
public class Tag: NSObject {
    ...
    public var tagClass:      TagClass      = .universal
    public var tagType:       TagType       = .primitive
    public var tagIdentifier: TagIdentifier = .leadingOctetLongIdentifier
    ...
    public override func isEqual(_ other: Any?) -> Bool {
        guard let other = other as? Tag else { return false }
    ...
    public static func tagClass(fromChar byte: UInt8) -> TagClass {
        let b = UInt(byte)
        if (b & TagClass.contextPrivate.rawValue) == TagClass.contextPrivate.rawValue {
            return .contextPrivate
        } else if (b & TagClass.application.rawValue) == TagClass.application.rawValue {
            return .application
        } else if (b & TagClass.contextSpecific.rawValue) == TagClass.contextSpecific.rawValue {
    ...
    public static func tagIsPrimitive(_ byte: UInt8) -> Bool {
        return (UInt(byte) & TagType.constructed.rawValue) != 0
    }

    public static func tagIdentifier(_ byte: UInt8, firstByte isFirstByte: Bool) -> UInt8 {
        return isFirstByte ? (byte & 0x1F) : (byte & 0x7F)
    }

    public static func tagIdentifierIsLastByte(_ byte: UInt8) -> Bool {
        return (byte & 0x80) == 0
    }

    public static func isValid(_ byte: Int) -> Bool {
        let byteNumber = 1
        if byteNumber <= 2 && byte == 0x00 { return false }
        if byteNumber == 2 && (byte == 0x1e || byte == 0x80) { return false }
        return true
    }

    public static func isConstructed(_ byte: Int) -> Bool {
        return getEncoding(byte) == 0x20
    }

    public static func getEncoding(_ byte: Int) -> Int {
        return byte & 0x20
    }

    public static func isMultiByte(_ byte: Int) -> Bool {
        return (byte & 0x1F) == 0x1F
    }

    public static func isLast(_ byte: Int) -> Bool {
        return (byte >> 7) == 0x00
    }

    public static func tagName(_ tag: String) -> String {
        return tagList[tag.uppercased()] ?? "Unknown"
    }

    private static let tagList: [String: String] = [
        "42"   : "Issuer Identification Number (IIN)",
        "4F"   : "Application Identifier (AID) from card",
        ...
        "9F06" : "Application Identifier (AID) from terminal",
