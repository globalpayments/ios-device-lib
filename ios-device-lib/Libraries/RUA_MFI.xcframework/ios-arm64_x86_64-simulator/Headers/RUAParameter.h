...
typedef NS_ENUM(NSInteger, RUAParameter) {
    ...
    /**
     ...
     0x00 = ICC referral processed offline, in the case of an ICC-initiated referral, after an auth code has been obtained.  The tag ì8Aî will be set by the EMVL2 kernel.
     0x01 = Online completed (approved or rejected by the host).  The tag ì8Aî must be set to the value from the host.
     0x02 = Unable to go online (comms failure, or declined by merchant)
     0x03 = Issuer reference, Approved
     0x04 = Issuer reference, Declined
     0x05 = Strong Customer Auth: Switch Interface
     0x06 = Strong Customer Auth: ask for PIN
     0x07 = Prompt for card again
     */
    ...
    /**
     Terminal Transaction Qualifiers, variable length 2-4 bytes <br>
     Terminal Transaction Qualifiers is a Contactless only terminal data element indicating capabilities and transaction-specific requirements of the terminal.<br>
     */
	RUAParameterTerminalTransactionQualifiers = 468,
	...
	*/
    ...
    /**
     ...
     0x02 = force online PIN as CVM only for AID with Domestic property set to 1
     0x03 = Don’t Prompt for online PIN if under CVM limit (only valid for CUP AIDs)
     0x04 = Don’t Prompt for Online PIN (only update the CVM result in DF6F). This value is only applicable to contactless transactions.
           Can be present in command parameter or in Aids TLV data. If present in both,  Command parameter got priority .
     */
    ...
    /**
     DF8244 :
     * In order to support DSP2 European rules & allow passing Visa performance test
     *
     * | Byte	| Description	| Format	| ** 	| Value														| example	|
     * | ---	| ---			| ---		| ---	| ---														| ---		|
     * | 1		| Magstripe		| b			|		| Define if fallback to Magstripe is allowed or not			| 0xFE		|
     * |		| fallback		|			|		| 00 -> not allowed error is directly returned on chip		|			|
     * |		| Allowed		|			|		| power up issue											|			|
     * |		|				|			|		| FE -> Keep default (allowed)								|			|
     * |		|				|			|		|															|			|
     * | 1		| Retry number	| b			|		| Define retry number (have priority on USCFG				| 0x03		|
     * |		|				|			|		| FALLBACK_NUMBER parameter)								|			|
     * |		|				|			|		| 00 -> no retry											|			|
     * |		|				|			|		| 01 -> FD number of retry before error						|			|
     * |		|				|			|		| FE -> Keep default from Config file						|			|
     * |		|				|			|		| FF -> infinite number of retry							|			|
     * |		|				|			|		|															|			|
     * | 1		| Mag delay		| b			|		| Add delay to reduce NFC sensitivity and avoid NFC			| 0x32		|
     * |		|				|			|		| picking up card being inserted.							|			|
     * |		|				|			|		| 00 / FF -> no delay										|			|
     * |		|				|			|		| 01 -> FD delay in 10ms									|			|
     * |		|				|			|		| FE -> Keep default from Config file						|			|
     * |		|				|			|		|															|			|
     * | 1		| Track2		| b			|		| Define if Track2 is Mandatory or not						| 0x01		|
     * |		| Optional		| /optional	|		| 00 -> mandatory ( default)								|			|
     * |		|				|			|		| 01 -> Track 2 optional									|			|
     * |		|				|			|		| FE -> Keep default from Config file						|			|
     * |		|				|			|		|															|			|
     * | 1		| Discard		| b			|		| Define if Track3 added to encrypted data or discarded		| 0x01		|
     * |		| track3		| /optional	|		| 00 -> Track 3 if present goes to encrypted data (default)	|			|
     * |		|				|			|		| 01 -> Track 3 Discarded									|			|
     * |		|				|			|		| FE -> Keep default from Config file						|			|
     *  */
    ...
    */
    ...
    RUAParameterSecondTapIndicator = 621,
    
    // EMV Tag 9F24 to PAR Data. This element is comprised of 29 alphanumeric characters - 4 representing a BIN Controller Identifier assigned by EMVCo and 25 representing a unique identifier which corresponds to a PAN. This enables merchants to recognise transactions issued with different payment tokens as belonging to the same PAN if they have a common PAR.
    RUAParameterPaymentAccountReference = 622,

    //RUAParameterDateTimeCapabilitiesExclusiveUseState = 623,
    //RUAParameterDateTimeCapabilitiesLockState = 624,

    /*RUAParameterCustomerScriptsVersion = 625,
    RUAParameterLevel1ModuleVersion = 626,
    RUAParameterLevel2ModuleVersion = 627,
    RUAParameterKernelModuleVersion = 628,
    RUAParameterE2EEModuleVersion = 629,
    RUAParameterAppleVASModuleVersion = 630,
    RUAParameterCustomerScriptsHash = 631,
    RUAParameterLevel1ModuleHash = 632,
    RUAParameterLevel2ModuleHash = 633,
    RUAParameterKernelModuleHash = 634,
    RUAParameterE2EEModuleHash = 635,
    RUAParameterAppleVASModuleHash = 636,
    RUAParameterUSRBuildId = 637,*/

    RUAParameterOverrideTerminalDecisionScreen = 638,
 ...
 */
	...
	/** [TODO] CustomerScriptsVersion documentation is missing. */
	RUAParameterCustomerScriptsVersion = 639,
	/** [TODO] Level1ModuleVersion documentation is missing. */
	RUAParameterLevel1ModuleVersion = 640,
	/** [TODO] Level2ModuleVersion documentation is missing. */
	RUAParameterLevel2ModuleVersion = 641,
	/** [TODO] KernelModuleVersion documentation is missing. */
	RUAParameterKernelModuleVersion = 642,
	/** [TODO] E2EEModuleVersion documentation is missing. */
	RUAParameterE2EEModuleVersion = 643,
	/** [TODO] AppleVASModuleVersion documentation is missing. */
	RUAParameterAppleVASModuleVersion = 644,
	/** [TODO] CustomerScriptsHash documentation is missing. */
	RUAParameterCustomerScriptsHash = 645,
	/** [TODO] Level1ModuleHash documentation is missing. */
	RUAParameterLevel1ModuleHash = 646,
	/** [TODO] Level2ModuleHash documentation is missing. */
	RUAParameterLevel2ModuleHash = 647,
	/** [TODO] KernelModuleHash documentation is missing. */
	RUAParameterKernelModuleHash = 648,
	/** [TODO] E2EEModuleHash documentation is missing. */
	RUAParameterE2EEModuleHash = 649,
	/** [TODO] AppleVASModuleHash documentation is missing. */
	RUAParameterAppleVASModuleHash = 650,
	/** [TODO] USRBuildId documentation is missing. */
	RUAParameterUSRBuildId = 651,
	/** DF8228: APDU Answer */
	RUAParameterAPDUAnswer = 652,
	/** [TODO] DateTimeCapabilitiesLockState documentation is missing. */
	RUAParameterDateTimeCapabilitiesLockState = 653,
	/** [TODO] DateTimeCapabilitiesExclusiveUseState documentation is missing. */
	RUAParameterDateTimeCapabilitiesExclusiveUseState = 654,
	/** [TODO] First6PANDigits documentation is missing. */
	RUAParameterFirst6PANDigits = 655,
	/** [TODO] VirtualTrack5 documentation is missing. */
	RUAParameterVirtualTrack5 = 656,
	/** Power cycles */
	RUAParameterPowerCycles = 657,
	/** Key hit */
	RUAParameterKeyHit = 658,
	/** MAG Stripe Swipes */
	RUAParameterMAGStripeSwypes = 659,
	/** audio connector in */
	RUAParameterAudioConnectorIn = 660,
	/** USB connections */
	RUAParameterUSBConnections = 661,
	/** MAG Swipe failures */
	RUAParameterMAGSwypeFailures = 662,
	/** Fallback to dip */
	RUAParameterFallbackToDip = 663,
	/** Total dip times */
	RUAParameterTotalDipTimes = 664,
	/** IC power-on failures */
	RUAParameterICPowerOnFailures = 665,
	/** IC APDU failures */
	RUAParameterICAPDUFailures = 666,
	/** RF-WUPA calls */
	RUAParameterRFWUPACalls = 667,
	/** RF activation failures */
	RUAParameterRFActivationFailures = 668,
	/** RF APDU failure */
	RUAParameterRFAPDUFailures = 669,
	/** Times charge */
	RUAParameterTimesCharge = 670,
	/** BT disconnections */
	RUAParameterBTDisconnections = 671,
	/** Battery out */
	RUAParameterBatteryOut = 672,
	/** Complete charge */
	RUAParameterCompleteCharge = 673,
	/** Commands processed */
	RUAParameterCommandsProcessed = 674,
	/** 
	 * DF8074: product name
	 * 
	 * product name 
	 *   ASCII format no NULL at end, variable length up to 256 characters. 
	 *  */
    RUAParameterProductName = 675,
	/** DF8164: extended POS entryMode
	 * 
	 * 2 bytes : 
	 *   1st byte : Standard POS entry mode : copy of 9F39 EMV  tag 
	 *   2nd byte :  for MagStripe  Fallback Reason Code 
	 * 
	 * "00": Normal (for MSR-only cards) 
	 * "01": Chip powered on but not responding 
	 * "02": No mutually supported application 
	 * "03": Failed to read chip data 
	 * "04": Mandatory Without data 
	 * "05": CVM command response failed 
	 * "06": EMV command set incorrectly */
	RUAParameterExtendedPOSEntryMode = 676,
	/** DF8132: Minimum Relay Resistance Grace Period */
	RUAParameterMinimumRelayResistanceGracePeriod = 677,
	/** DF8133: Maximum Relay Resistance Grace Period */
	RUAParameterMaximumRelayResistanceGracePeriod = 678,
	/** [TODO] BatteryInfoStatus documentation is missing. */
	RUAParameterBatteryInfoStatus = 679,
	/** [TODO] BatteryInfoPowerType documentation is missing. */
	RUAParameterBatteryInfoPowerType = 680,
	/** [TODO] BatteryInfoCapacity documentation is missing. */
	RUAParameterBatteryInfoCapacity = 681,
	/** [TODO] BatteryInfoCapacityLevel documentation is missing. */
	RUAParameterBatteryInfoCapacityLevel = 682,
	/** [TODO] BatteryInfoCycles documentation is missing. */
	RUAParameterBatteryInfoCycles = 683,
	/** [TODO] BatteryInfoTemperature documentation is missing. */
	RUAParameterBatteryInfoTemperature = 684,
// endregion PreGenerator

    RUAParameterDeviceInfo = 685,

    RUAParameterMifareUltralightUUID = 686,

    RUAParameterMifareUltralightLanguageCode = 687,

    RUAParameterMifareUltralightText = 688,

    /** DF8104: Balance Read Before GenAC
     ...
     */
    RUAParameterBalanceReadBeforeGenAC = 993,
    /** DF8105: Balance Read After GenAC
     * The presence of Balance Read After Gen AC in the TLV Database is an indication to the Kernel to read the offline balance from the Card before the GENERATE AC command.
     */
    RUAParameterBalanceReadAfterGenAC = 994,
    /** 9F50: Offline Accumulator Balance
     * Represents the amount of offline spending available in the Card.
     */
    RUAParameterOfflineAccumulatorBalance = 995,
    /**
